...
import { Github, ExternalLink, Mail, Linkedin, Phone, Download, MapPin } from "lucide-react";
...
const PROJECTS = [
  {
    ...
    blurb: "Scrapes Israeli outlets (RSS/Selenium), clusters coverage, and visualizes trends.",
  ...
  },
  {
    ...
    blurb: "Real-time sensor ingestion, anomaly alerts, and dashboards for building risk.",
  ...
  },
  {
    ...
    blurb: "ML classification on Kaggle dataset to surface risk patterns and signals.",
  ...
  },
  {
    ...
    blurb: "Pipeline that extracts and structures job postings for market insights.",
    ...
    <main className="min-h-screen bg-neutral-50 text-neutral-900">
      ...
      <header className="sticky top-0 z-50 backdrop-blur bg-white/70 border-b border-neutral-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <a href="#hero" className="font-semibold tracking-tight text-lg">{INFO.name}</a>
          <nav className="flex items-center gap-2 text-sm">
            {[
              ...
              <a
                ...
                className={`px-3 py-1.5 rounded-full transition-all ${
                  active === link.id ? "bg-neutral-900 text-white" : "hover:bg-neutral-100"
                }`}
              >
                {link.label}
              </a>
            ...
            <a href={INFO.resumeUrl} className="ml-2">
              <Button className="rounded-2xl shadow-sm"><Download className="h-4 w-4 mr-2"/>Resume</Button>
            </a>
          </nav>
        </div>
      </header>
          ...
          <div className="md:col-span-2">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <a href={`mailto:${INFO.email}`}>
                <Button variant="secondary" className="rounded-2xl"><Mail className="h-4 w-4 mr-2"/>Email</Button>
              </a>
              <a href={INFO.linkedin} target="_blank" rel="noreferrer">
                <Button className="rounded-2xl"><Linkedin className="h-4 w-4 mr-2"/>LinkedIn</Button>
              </a>
          ...
          <div className="md:ml-auto">
            <Card className="rounded-2xl shadow-sm">
              ...
              <CardContent className="text-sm space-y-2">
                <p className="flex items-center gap-2"><MapPin className="h-4 w-4"/>{INFO.location}</p>
                <p className="flex items-center gap-2"><Phone className="h-4 w-4"/>{INFO.phone}</p>
                <p className="flex items-center gap-2"><Mail className="h-4 w-4"/>{INFO.email}</p>
              </CardContent>
            </Card>
          </div>
        </div>
            ...
            <Card key={i} className="rounded-2xl hover:shadow-md transition-shadow">
              ...
              <CardContent>
                ...
                <ul className="flex flex-wrap gap-2 mb-4">
                  {p.tech.map((t) => (
                    <li key={t} className="text-xs bg-neutral-100 rounded-full px-2.5 py-1">{t}</li>
                  ))}
                </ul>
                <div className="flex items-center gap-2">
                  <a href={p.live} target="_blank" rel="noreferrer">
                    <Button size="sm" className="rounded-xl"><ExternalLink className="h-4 w-4 mr-1"/>Live</Button>
                  </a>
                  <a href={p.repo} target="_blank" rel="noreferrer">
                    <Button size="sm" variant="outline" className="rounded-xl"><Github className="h-4 w-4 mr-1"/>Code</Button>
                  </a>
                </div>
              </CardContent>
            </Card>
      ...
      <section id="contact" className="max-w-6xl mx-auto px-4 py-12">
        <div className="rounded-3xl p-6 border bg-white shadow-sm">
          ...
          <p className="text-neutral-700 mb-4">Open to full-time or freelance roles. Best way to reach me: email or LinkedIn.</p>
          <div className="flex flex-wrap gap-3">
            <a href={`mailto:${INFO.email}`}>
              <Button className="rounded-2xl"><Mail className="h-4 w-4 mr-2"/>Email me</Button>
            </a>
            <a href={INFO.linkedin} target="_blank" rel="noreferrer">
              <Button variant="secondary" className="rounded-2xl"><Linkedin className="h-4 w-4 mr-2"/>Connect</Button>
            </a>
            <a href={INFO.github} target="_blank" rel="noreferrer">
              <Button variant="outline" className="rounded-2xl"><Github className="h-4 w-4 mr-2"/>GitHub</Button>
            </a>
        ...
        <footer className="text-center text-sm text-neutral-500 py-8">© {new Date().getFullYear()} {INFO.name}</footer>
      </section>
    </main>
  );
}
